...
# -------------------------------
motivations = [
    "지금 이 순간이 미래를 만든다!", "작은 노력이 큰 변화를 만든다.", "멈추지 않으면 늦어도 도착한다.",
    "오늘의 땀이 내일의 성적표다.", "넌 할 수 있다, 이미 절반은 했다!", "어제보다 나은 내가 되자.",
    "공부는 배신하지 않는다.", "집중하는 1시간이 놀라운 결과를 만든다.", "포기하지 않는 자가 결국 이긴다.",
    "작심삼일? 삼일마다 다시 시작하면 된다.", "끝까지 버티는 자가 승리한다.", "후회 없는 하루를 보내자.",
    "너의 가능성은 무한하다.", "오늘의 선택이 내일의 성적을 결정한다.", "한 페이지라도 더 보자.",
    "작은 습관이 합격을 만든다.", "실패는 성공으로 가는 과정이다.", "남과 비교 말고 어제의 나와 비교하자.",
    "넌 생각보다 강하다.", "합격은 노력하는 자의 것.", "미래의 너가 오늘의 너에게 감사할 것이다.",
    "오늘 할 일을 내일로 미루지 말자.", "공부는 재능보다 끈기다.", "시간은 칼이다, 현명하게 써라.",
    "버티는 자가 결국 웃는다.", "노력은 배신하지 않는다.", "오늘을 이겨내자.", "꾸준함이 가장 큰 무기다.",
    "10분 더! 그게 합격을 만든다.", "포기하지 않는 한 실패는 없다."
]
...
bg_choice = st.sidebar.radio("배경 선택", ["흰", "빨", "주", "노", "검", "무지개", "갤러리에서 업로드"])

if bg_choice == "갤러리에서 업로드":
    bg_file = st.sidebar.file_uploader("이미지 업로드", type=["png", "jpg", "jpeg"])
    if bg_file:
